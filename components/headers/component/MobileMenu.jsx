...
export default function MobileMenu() {
  ...
  const isMenuActive = (menu) => {
    ...
    if (menu.href) {
      ...
      // Original logic for internal links
    ...
    if (menu.subItems) {
      menu.subItems.forEach((el) => {
        if (el.href) {
          if (!el.href.startsWith('http') && pathname.split("/")[1] == el.href?.split("/")[1]) {
            isActive = true;
          }
        }
        if (el.subItems) {
          el.subItems.map((elm) => {
            if (elm.href) {
              if (!elm.href.startsWith('http') && pathname.split("/")[1] == elm.href?.split("/")[1]) {
                isActive = true;
              }
            }
          });
        ...
        <div className="panel">
          ...
          <ul className="nav-y gap-narrow fw-medium fs-6 uc-nav" data-uc-nav="">
            ...
            <li className="mt-3">
                <a
                    ...
                    role="button"
                    className="btn btn-sm btn-primary text-white text-none d-inline-flex w-full justify-center"
                >
                    Try a free class
                </a>
            </li>
          ...
          >
            <div className="vstack gap-1">
              <span className="fs-7 opacity-60">Select mode:</span>
